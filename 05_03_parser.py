...
# Имя файла
filename = '05-03 оборот розничной торговли непродовольственными.xls'
...
# Полный путь к файлу
full_filename = folder + "/05 торговля/" + filename
print(full_filename)

# functions_start
def parse_data(file, xfile_id, code):
...
try:
    ...
    for i in range(4):
        code = codes[i]
        file = pd.read_excel(full_filename, header=[0, 1, 2, 3], sheet_name=i, index_col=0)
        file.drop(file.tail(len(file.index) - subjects_count).index, inplace=True)
        parse_data(file, xfile_id, code)
